...
class Flatten:
    ...
    def forward(self, input):
        ...
        flattened = np.empty((input.shape[0], seq.prod(input.shape[1:])))
...
class Dense:
    ...
    def forward(self, input):
        ...
        self.last_output = seq.dot(input, self.weights) + self.biases
        return self.last_output

    def backprop(self, d_L_d_out, lr):
        d_L_d_input = seq.dot(d_L_d_out, self.weights.T)

        # Update weights and biases
        d_L_d_weights = seq.dot(self.last_input.T, d_L_d_out)
...
class SoftMaxCrossEntropy:
    ...
    def _softmax(self, z: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        sum_exp = np.sum(seq.exp(z))
        return seq.exp(z) / sum_exp
    ...
    def _cross_entropy_batch(self, y: np.ndarray, y_hat: np.ndarray) -> float:
        ...
        log_y_hat = seq.log(y_hat + epsilon)
